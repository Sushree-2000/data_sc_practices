...
def sampling(args):
    ...
    epsilon = K.random_normal(shape=(batch, Z_DIM), mean=0., stddev=1.0)
...
class ConvVAE:
    ...
    def _build(self):
        ...
        vae_c2 = Conv2D(filters=CONV_FILTERS[1], kernel_size=CONV_KERNEL_SIZES[1], strides=CONV_STRIDES[1], activation=CONV_ACTIVATIONS[1])(vae_c1)
        # Creating the third convolutional layer of the Encoder
        vae_c3 = Conv2D(filters=CONV_FILTERS[2], kernel_size=CONV_KERNEL_SIZES[2], strides=CONV_STRIDES[2], activation=CONV_ACTIVATIONS[2])(vae_c2)
        # Creating the fourth convolutional layer of the Encoder  
        vae_c4 = Conv2D(filters=CONV_FILTERS[3], kernel_size=CONV_KERNEL_SIZES[3], strides=CONV_STRIDES[3], activation=CONV_ACTIVATIONS[3])(vae_c3)
        # Flattening the last convolutional layer so we can input it in the dense layers
        vae_z_in = Flatten()(vae_c4)
        # Using two separate files to calculate z_mean and z_log_var
        ...
        vae_d1 = Conv2DTranspose(filters=CONV_T_FILTERS[0], kernel_size=CONV_T_KERNERL_SIZES[0], strides=CONV_T_STRIDES[0], activation=CONV_T_ACTIVATIONS[0])
        # Creating the first decoder layer
        vae_d1_model = vae_d1(vae_z_out_model)
        # Defining the second transposed convolutional layer
        vae_d2 = Conv2DTranspose(filters=CONV_T_FILTERS[1], kernel_size=CONV_T_KERNERL_SIZES[1], strides=CONV_T_STRIDES[1], activation=CONV_T_ACTIVATIONS[1])
        # Creating the second decoder layer
        vae_d2_model = vae_d2(vae_d1_model)
        # Defining the third transposed convolutional layer
        vae_d3 = Conv2DTranspose(filters=CONV_T_FILTERS[2], kernel_size=CONV_T_KERNERL_SIZES[2], strides=CONV_T_STRIDES[2], activation=CONV_T_ACTIVATIONS[2])
        # Creating the third decoder layer
        vae_d3_model = vae_d3(vae_d2_model)
        # Defining the fourth transposed convolutional layer
        vae_d4 = Conv2DTranspose(filters=CONV_T_FILTERS[3], kernel_size=CONV_T_KERNERL_SIZES[3], strides=CONV_T_STRIDES[3], activation=CONV_T_ACTIVATIONS[3])
        # Creating the fourth decoder layer
        vae_y = vae_d4(vae_d3_model)
        ...
        def vae_kl_loss(y_true, y_pred):
            ...
            # kl_loss = K.maximum(kl_loss, 0.5 * Z_DIM)
            # return kl_loss
    ...
    def train(self, data, validation_split = 0.2):
        ...
        self.model.fit(data, data, epochs=EPOCHS, batch_size=BATCH_SIZE, shuffle=True, validation_split=validation_split, callbacks=callbacks_list)
